...
def load_images(image_paths):
    ...
    imagePaths = image_paths.values()
    value_iterator = iter(imagePaths)
    T1 = next(value_iterator)
    GroundTruth = next(value_iterator)

    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(T1, sitk.sitkFloat32),  # todo: modify here
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(GroundTruth, sitk.sitkUInt8)  # todo: modify here
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_gt = sitk.Resample(gt_img, atlas_img, registration.transform, sitk.sitkNearestNeighbor, 0.0, gt_img.GetPixelID())  # todo: modify here
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    filter = MinMaxRescaleFilter() 
    filter_params = MinMaxRescaleFilterParams(new_min_val, new_max_val) 
    minmax_rescaled_img = filter.execute(image_dict.get(structure.BrainImageTypes.T1w), filter_params)
...
def extract_feature_median_t1(image_dict):
    ...
    median_img = filter.execute(image_dict.get(structure.BrainImageTypes.T1w))  # todo: modify here
