...
\subsection{Medical Image Analysis pipeline}
	The Medical Image Analysis (MIA) pipeline taught in the MIA Lab lectures follow the sequence of firstly perform {\itshape Registration} to T1- and T2-weighted images (T1w and T2w image), then {\itshape Pre-Process}-methods. Additionally {\itshape Feature Extraction} followed by the {\itshape Classification} of the images are performed. At the end {\itshape Post-Processing}-methods are performed. Eventually, the segmentation of one patient's data set is achieved. 
	During {\itshape Registration} a floating image is transformed with an affine transformation, such that it is similar to a given reference image.

\smallskip
	In this project no {\itshape Registration}, {\itshape Feature Extraction}, {\itshape Classification} nor {\itshape Post-Processing} has been applied to the images. The phase of the MIA pipeline analysed and applied to the images was {\itshape Pre-Processing}.
	...
	A skull stripping, as well as several different normalization methods were applied. 
...
\subsection{Data}
	...
	Overall, the dataset consisted of 30 MRI patient images, out of which 20 were used for training and 10 for testing the model. 
	From each patient the ground truth image, a T1w and a T2w image were available. The images were generated by a 3 Tesla MRI. 
	...
	Each MRI file is of the size 118x118x217 pixels.

\textbf{From MIA Lab slides: Details on data pre processing
\begin{itemize}
	\item T1w , T2w in native space
	\item Both bias field corrected
	\item Brain mask available
	\item Transformation to atlas space available
	\end{itemize}
}
